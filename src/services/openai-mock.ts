// OpenAI service that switches between real API and mock
import { mockGenerateMessageReply, mockAnalyzeMessageUrgency } from './mock-openai';
...
if (!USE_MOCK) {
  ...
  console.log('🧪 Using MOCK OpenAI (set OPENAI_API_KEY to use real API)');
}
...
export const generateMessageReply = async (
  ...
  if (USE_MOCK) {
    return mockGenerateMessageReply(agentName, contactName, contactRole, propertyInfo, clientMessage, voiceInput, tone);
  }
...
export const analyzeMessageUrgency = async (message: string): Promise<'low' | 'medium' | 'high'> => {
  if (USE_MOCK) {
    return mockAnalyzeMessageUrgency(message);
  }
